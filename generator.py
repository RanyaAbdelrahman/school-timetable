...
def generate_timetable():
    ...
    for _, row in df_teachers.iterrows():
        ...
        off_days = clean_off_days(row["OffDays"]) if "OffDays" in df_teachers.columns else []
        valid_off_days = [d for d in off_days if d in days]
        teacher_off_days[teacher_name] = valid_off_days

        teacher_unwanted_periods[teacher_name] = clean_unwanted_periods(row["UnwantedPeriods"]) if "UnwantedPeriods" in df_teachers.columns else []
        supervision_days = clean_supervision_days(row["الإشراف"]) if "الإشراف" in df_teachers.columns else []
        teacher_supervision_days[teacher_name] = [d for d in supervision_days if d in days]
    ...
    # ========================================================
    # 🌟 عقوبة هائلة ذكية لتوزيع المواد ومنع تركزها (تجنب الإغلاق Infeasible)
    # ========================================================
    ...
    for (class_name, subject), items in class_subject_groups.items():
        if num_days <= 1:
            continue
        total_weekly = sum(int(item["w"]) for item in items)
        ideal_max = math.ceil(total_weekly / num_days)
        ...
        for d in range(num_days):
            ...
            if day_vars:
                day_load = model.NewIntVar(0, num_periods, f"load_{class_name}_{subject}_{d}")
                model.Add(day_load == sum(day_vars))

                # عقوبة صارمة جداً لكل حصة إضافية تتجاوز الحد المثالي لتجبر النظام على التوزيع العادل دون منع الحل
                over_limit = model.NewIntVar(0, num_periods, f"over_{class_name}_{subject}_{d}")
                model.Add(over_limit >= day_load - ideal_max)
                model.Add(over_limit >= 0)
                objective_terms.append(over_limit * (-500000))

    # باقي القيود وأهداف التوازن الأخرى...
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        print()
        print("=" * 60)
        print("✅ تم توليد الجدول بنجاح تام!")
        print("=" * 60)
        ...
        safe_school_name = re.sub(r'[\\/:*?"<>|]+', "_", str(school_name).strip()).strip(" .") or "مدرسة"
        ...
        # تصدير ملف الإكسيل الشامل وملفات الفصول والمعلمين
            ...
            for t in teachers:
                ...
                for day_name in days:
                    if day_name in off_days_for_t:
                        for p_col in periods: pivot_t.loc[day_name, p_col] = "إجازة"
                    else:
                        for p_col in periods:
                            if pd.isna(pivot_t.loc[day_name, p_col]): pivot_t.loc[day_name, p_col] = "راحة"
