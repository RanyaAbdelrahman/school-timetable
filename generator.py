...
def format_excel_workbook(file_path):
    ...
    for sheetname in wb.sheetnames:
        ...
        if "_" in sheetname:
            ...
            sheet_type, title_val = sheetname.split(
                "_",
                1
            )
...
def generate_timetable():
    ...
    if not os.path.exists(excel_file):
        ...
        print(
            "❌ لم يتم العثور على ملف inputs.xlsx"
        )
    ...
    try:
        ...
        print(
            "❌ حدث خطأ أثناء قراءة inputs.xlsx"
        )
    ...
    for _, row in df_teachers.iterrows():
        ...
        if (
            "MaxOffDays" in df_teachers.columns
            and pd.notna(
                row.get("MaxOffDays")
            )
        ):
    ...
    # ⭐ منع تعارض المعلم
    #
    # المعلم لا يمكن أن يكون في فصلين
    # في نفس اليوم والحصة
    # ========================================================
    ...
    # ⭐ OFF DAYS
    #
    # أي معلم في Assignment له OFF
    # لا يأخذ الحصة في هذا اليوم
    # ========================================================
    ...
    #
    # مهم:
    # لا نمنع أكثر من حصة لنفس المادة في اليوم.
    #
    # لأن:
    # 8 حصص يمكن أن تحتاج أكثر من حصة في بعض الأيام.
    # ========================================================
    ...
    # ========================================================
    # تفضيل توزيع كل مادة على أيام مختلفة
    # ========================================================
    ...
    for item in clean_assignments:
        ...
        for d in range(num_days):
            ...
            day_has_subject = model.NewBoolVar(
                f"day_has_{idx}_{d}"
            )
            ...
            # نفضل توزيع المادة على أيام أكثر
            objective_terms.append(
                day_has_subject * 50
            )
    ...
    for item in clean_assignments:
        ...
        for d in range(num_days):
            ...
            for p in range(
                ...
                model.Add(
                    both_lessons <= current_var
                )

                model.Add(
                    both_lessons <= next_var
                )
    ...
    if status in (
        ...
        if status == cp_model.INFEASIBLE:
            ...
            print(
                "السبب: توجد قيود متعارضة "
                "تجعل الجدول مستحيلاً."
            )

        elif status == cp_model.UNKNOWN:

            print(
                "البرنامج لم يصل إلى حل خلال الوقت المحدد."
            )

        else:

            print(
                f"حالة Solver: {status}"
            )
        ...
        print(
            "1️⃣ عدد الحصص لكل Assignment"
        )

        print(
            "2️⃣ تعارضات المعلمين"
        )
